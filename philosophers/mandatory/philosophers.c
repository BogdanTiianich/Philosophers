...
/*   Updated: 2022/04/21 21:41:58 by hbecki           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_philo_eating(t_data data)
{
	...
	pthread_mutex_unlock(data.mutex);//needs to be checked
...
int	ft_start_dinning(t_data data)
{
	...
	ft_time_from_start(data.rules->start_time), data.philos->number);
	(*data.avail_forks)[data.philos->forks_i_need[0]] = 0;
	(*data.avail_forks)[data.philos->forks_i_need[1]] = 0;
	pthread_mutex_unlock(data.mutex);
	life_and_death = ft_philo_eating(data);
	(*data.avail_forks)[data.philos->forks_i_need[0]] = 1;
...
void	ft_run_game(t_data **data, pthread_t *threads, t_rules *rules, int **avail_forks)
{
	...
	while (vars.i < rules->number_of_phils)
	{
		...
		(*data[vars.i]).philos->forks_i_need = (int *)malloc(sizeof(int) * 2);
		if ((*data[vars.i]).philos->forks_i_need == NULL)
		{
			ft_errors(rules);
		}
		ft_define_forks_i_need((*data[vars.i]).philos, rules->number_of_phils);
